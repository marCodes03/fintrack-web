...
import { ApiService, Transaction, Account, RefExpenseCategory, BudgetPlan } from '../../services/api.service';
import { AuthService } from '../../services/auth.service';
import { HeaderComponent } from '../header/header.component';
import { ResetPasswordComponent } from '../auth/reset-password/reset-password.component';
import { ConfirmDeleteModalComponent } from '../confirm-delete-modal/confirm-delete-modal.component';
import { ExpenseSliderComponent } from './expense-slider.component';
import { DatePipe, CurrencyPipe } from '@angular/common';

@Component({
  selector: 'app-expenses',
  standalone: true,
  imports: [HeaderComponent, ResetPasswordComponent, ConfirmDeleteModalComponent, ExpenseSliderComponent, DatePipe, CurrencyPipe, FormsModule],
...
export class ExpensesComponent implements OnInit {
  ...
  protected readonly typeFilter = signal<string>('ALL'); // ALL, FIXED, VARIABLE
  ...
  protected category = '';
  protected accountId = '';
  protected expenseType: 'FIXED' | 'VARIABLE' = 'VARIABLE';
  protected expenseDate: string = new Date().toISOString().split('T')[0];
  ...
  protected readonly totalExpenses = computed(() => {
    return this.filteredTransactions().reduce((sum, tx) => sum + Math.abs(tx.amount), 0);
  });

  protected readonly filteredTransactions = computed(() => {
    let items = this.transactions().filter(tx => {
      // Expense Type
      if (this.typeFilter() !== 'ALL' && (tx.expenseType || 'VARIABLE') !== this.typeFilter()) {
        return false;
      }
      // Category
  ...
  loadData(): void {
    ...
    this.apiService.getExpenseCategories().subscribe({
      next: (res) => {
        if (res.success) {
          this.expenseCategories.set(res.data.filter(c => c.isActive));
  ...
  startEdit(tx: Transaction): void {
    ...
    this.expenseType = (tx.expenseType as 'FIXED' | 'VARIABLE') || 'VARIABLE';
    this.expenseDate = tx.expenseDate ? tx.expenseDate.split('T')[0] : new Date().toISOString().split('T')[0];
    this.isSliderOpen.set(true);
  }
  ...
  onSaved(): void {
    this.successMessage.set(this.isEditMode() ? 'Your expense details have been updated!' : 'Got it! Your expense has been recorded.');
    this.loadData();
  }

  handleDeleteExpense(tx: Transaction): void {
    this.errorMessage.set(null);
    this.successMessage.set(null);
  ...
  executeDelete(): void {
    ...
    this.http.delete(`${this.apiService.apiUrl}/transactions/${tx.id}`).subscribe({
      next: () => {
        this.isLoading.set(false);
        this.successMessage.set('Expense transaction deleted successfully.');
        this.closeDeleteModal();
        this.loadData();
      },
      error: (err) => {
        this.isLoading.set(false);
        this.errorMessage.set(err.error?.message || "Failed to delete the expense. Let's try that again.");
        this.closeDeleteModal();
      }
    });
  }
  ...
  executeBulkDelete(): void {
    ...
    this.http.post(`${this.apiService.apiUrl}/transactions/bulk-delete`, { ids }).subscribe({
      next: () => {
        this.isLoading.set(false);
        this.successMessage.set('All selected expense transactions have been deleted.');
      ...
      error: (err) => {
        this.isLoading.set(false);
        this.errorMessage.set(err.error?.message || "Failed to delete the selected expense transactions. Let's try that again.");
        this.closeBulkDeleteModal();
      }
    });
  }
  ...
  resetForm(): void {
    this.description = '';
    this.amount = null;
    this.category = this.expenseCategories().length > 0 ? this.expenseCategories()[0].name : '';
    this.expenseType = 'VARIABLE';
